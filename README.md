...
## 1. 프로젝트 소개
**PyAccountant**는 파이썬 프로그래밍 수업에서 배운 내용을 종합하여 만든 스마트 가계부 프로그램입니다. 
단순한 입출력뿐만 아니라, `Pandas`와 `Matplotlib`을 활용하여 사용자의 소비 패턴을 분석하고 시각화해주는 기능을 제공합니다.

## 2. 주요 기능 및 적용된 기술
* **객체 지향 프로그래밍 (Lecture 6):** `AccountBook` 클래스를 통해 데이터를 관리하고 기능을 모듈화했습니다.
* **파일 입출력 & 데이터 관리 (Lecture 8):** 모든 데이터는 `csv` 파일로 영구 저장되며, 프로그램 재실행 시에도 데이터가 유지됩니다.
