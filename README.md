...
## 1. 프로젝트 소개
**PyAccountant**는 파이썬 프로그래밍 수업에서 배운 내용을 종합하여 만든 스마트 가계부 프로그램이다. 
단순한 얼마를 쓰고 벌었는지뿐만 아니라 `Pandas`와 `Matplotlib`을 활용하여 사용자의 소비 패턴을 분석하고 시각화해주는 기능을 제공한다.

## 2. 선정 이유
가계부를 만들려고 할 때 일일이 타이핑해서 카테고리별로 분류하는 게 불편하고 한 달에 얼마나, 무엇에 돈을 사용하는지 한눈에 알기 어려워 이를 Python을 통해 간단하게 구현해보고 싶어서 이 주제를 선정하게 되었다. 

## 3. 주요 기능 및 적용된 기술
* **객체 지향 프로그래밍 (Lecture 6):** `AccountBook` 클래스를 통해 데이터를 관리하고 기능을 모듈화했습니다.
* **파일 입출력과 데이터 관리 (Lecture 8):** 모든 데이터는 `csv` 파일로 영구 저장되며, 프로그램 재실행 시에도 데이터가 유지됩니다.
* **데이터 분석과 시각화 (Lecture 9):** `Pandas`로 데이터를 그룹화(Groupby)하고 `Matplotlib`으로 파이 차트를 그려 소비 성향을 한눈에 파악합니다.
